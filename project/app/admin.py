...
class EmployeeRoleAdmin(admin.ModelAdmin):
    ...
    fieldsets = ((None, {"fields": ("RoleName",)}),)
...
class DomainAdmin(admin.ModelAdmin):
    ...
    list_filter = ("RegisteredDate",)
    fieldsets = ((None, {"fields": ("DomainName",)}),)
...
class PhoneBookAdmin(admin.ModelAdmin):
    ...
    list_editable = [
        ...
        "Phone2",
        "Station",
    ]  # Allow inline editing of 'Type' field
...
class AfflictionAdmin(admin.ModelAdmin):
    ...
    def view_report_icon(self, obj):
        if obj.Report:
            return format_html(
                '<a href="{0}" download title="Download Report"><i class="fas fa-file-alt text-success"></i></a>',
                obj.Report.url,
            )  # Change the class to the desired Font Awesome icon
        else:
            return None
...
class WorkMeetingAdmin(admin.ModelAdmin):
    ...
    def view_report_icon(self, obj):
        if obj.Report:
            return format_html(
                '<a href="{0}" download title="Download Report"><i class="fas fa-file-alt text-dark"></i></a>',
                obj.Report.url,
            )  # Change the class to the desired Font Awesome icon
        else:
            return None
...
class MahudhurioAdmin(admin.ModelAdmin):
    ...
    fieldsets = (
        ...
        ("Meeting Type & Unit", {"fields": ("MeetingType", "Unit", "MeetingDate")}),
        ("Additional Information", {"fields": ("LastUpdated", "RegisteredDate")}),
    )
